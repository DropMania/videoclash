<script>
    ...
    supabase.auth.onAuthStateChange((event, session) => {
        if (session) {
            ...
            let last_user_data_sync = localStorage.getItem('last_user_data_sync');
            if($user && (last_user_data_sync === null || getHoursDiff (last_user_data_sync, new Date() ) > 24  )  ){
                supabase.rpc('update_or_insert_user_data',{
                    ...
                    i_description:session.user.user_metadata.custom_claims.description,
